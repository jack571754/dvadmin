...
class FlowInfo(CoreModel):
    ...
    correlation_model_name = models.CharField(max_length=255, verbose_name="关联模型名称", null=True, blank=True)
    ...
    enter_fields = models.JSONField(blank=True, null=True,default=list, verbose_name='进入字段进行审批')
    rule_status = models.SmallIntegerField(default=1, choices=RULE_STATUS_CHOICES, verbose_name='规则配置状态', help_text='规则配置状态')
...
class FlowData(CoreModel):
    ...
    correlation_model = models.ForeignKey(ContentType, on_delete=models.CASCADE, verbose_name="关联模型",db_constraint=False,
                                          help_text="关联模型", null=True, blank=True, )
    object_id = models.CharField(max_length=255, verbose_name="对象ID", null=True, blank=True, )
    content_object = GenericForeignKey('correlation_model', 'object_id')
    correlation_model_name = models.CharField(max_length=255, verbose_name="关联模型名称", null=True, blank=True)
